# for i in $(seq 1 1 9); do echo "Rscript Cruise_Size_distribution.R $i synecho Thompson_9" | qsub -lwalltime=24:00:00,nodes=1:ppn=1 -N syn_size_dist$i -d.; done
...
tiff(paste(home,folder,cruise,"/Beads_",julian.day,".tiff",sep=""),compression="lzw", width=13.5, height=10, units='in',res=150)
		...
		par(mfrow=c(2,1))
		plot(beads[,"time"], beads[,paste(para,"_mode",sep="")],pch=1,col='grey', ylim=c(0,2^16), xlim=c(min(all.stat[,"time"],na.rm=T),max(all.stat[,"time"],na.rm=T)), xlab='time',ylab=paste("BEADS",para,"_mode",sep=""))
			smooth <- smooth.spline(beads[,"time"], beads[,paste(para,"_mode",sep="")], spar=spar)
			time.in.common <- subset(all.stat, time > min(beads[,'time'],na.rm=T) & time < max(beads[,'time'],na.rm=T))
		...
		plot(beads[,"time"], beads[,paste(para,"_mode",sep="")],pch=1,col='grey', ylim=c(0,2^16), xlim=c(min(stat[,"time"],na.rm=T),max(stat[,"time"],na.rm=T)), xlab='time',ylab=paste("BEADS",para,"_mode",sep=""), main=paste(julian.day))
		lines(smooth.beads, col= 'red', lwd=3)	
		abline(h=fsc_ref, lwd=3, lty=2)
...
#f<- filename[134]
			...
			for (f in filename) {
					...
					dens <- density(pop[,para], n=n.breaks,from=0, to=2^16, bw="SJ", kernel='gaussian',na.rm=T)
