...
matrix.conct.fast <- function(hr, Einterp, volbins, gmax, dmax, a, b, E_star){
		...
		stasis_ind <- seq(m+2,m^2-1,by=m+1) # indexing for matrix -goes down a column and then right to next column
		...
		for(t in 1:(1/dt)){
			...
			A[1,2:(j-1)] <- 2 * delta[2:(j-1)] # Top row; Small phytoplanktoin (i=2,..., j-1) are less than twice as big as the smallest size class, and so after division are put in the smallest size class.
...
determine.opt.para <- function(Vhists,N_dist,Edata,volbins){
		...
		mu_N <- matrix(nrow=1,ncol=length(timepoint))
			...
			for(hr in timepoint[-tail(timepoint,1)]){
					...
					Vproj[,hr+1] <- B %*% Vproj[,hr] # calculate the projected size-frequency distribution
						Vproj[dim(Vproj)[1],hr+1] <- NA
					Vproj[,hr+1] <- na.spline(Vproj[,hr+1])
						Nproj[dim(Nproj)[1],hr+1] <- NA
					Nproj[,hr+1] <- na.spline(Nproj[,hr+1])
					Vproj[,hr+1] <- Vproj[,hr+1]/sum(Vproj[,hr+1]) # normalize distribution
					mu_N[,hr+1] <- log(sum(Nproj[,hr+1]))-log(sum(Nproj[,hr]))
		...
		d.mu_N <- sum(mu_N, na.rm=T)/(25/length(mu_N))
